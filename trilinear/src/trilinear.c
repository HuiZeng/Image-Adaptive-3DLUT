                      ...
                      int lut_dim, int shift, float binsize, int width, int height, int batch)
{
    ...
    float * lut_flat = THFloatTensor_data(lut);
    float * image_flat = THFloatTensor_data(image);
    float * output_flat = THFloatTensor_data(output);

    // whether color image
    int channels = THFloatTensor_size(image, 1);
    if (channels != 3)
    {
        return 0;
    }
...
{
    // Grab the input tensor
    float * image_grad_flat = THFloatTensor_data(image_grad);
    float * image_flat = THFloatTensor_data(image);
    float * lut_grad_flat = THFloatTensor_data(lut_grad);
